...
def workflow_compress_imgs(cli_logger: Logger, file_path: str):
    cli_logger.debug(
    ...
    for jpg_file in jpg_file_list:
        ...
        cli_logger.debug(f"jpg file: {jpg_file}")
        cli_logger.debug(f"file size before {file_size_mb_before:.2f} MB")
        if file_size_mb_before > 1:
            mpic.compress_jpg(jpg_file, jpg_file, quality=30)
            file_size_mb_after = mio.get_file_size_mb(jpg_file)
            cli_logger.debug(f"file size after {file_size_mb_after:.2f} MB")
        else:
            cli_logger.debug(f"file with a small size skip for compressing")

    for png_file in png_file_list:
        file_size_mb_before = mio.get_file_size_mb(png_file)
        cli_logger.debug(f"png file: {png_file}")
        cli_logger.debug(f"file size before {file_size_mb_before:.2f} MB")
...
if __name__ == "__main__":
    workflow_compress_imgs(mlogger.global_logger, ".")
